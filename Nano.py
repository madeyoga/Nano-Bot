...
@bot.command(pass_context=True)
async def example(ctx, cmd : str):
    """ shows example usage of commands, -commands <cmd> """
    command = ""
    if cmd == "ping":
        command = "-ping"
    elif cmd == "fact":
        command = "-fact @tag_member"
    elif cmd == "clear":
        command = "-clear 20\nif no args then i will delete all the messages\n"
        command += "I Can only delete messages in the range of [2, 100] "
        command += "and if i have the permissions"
    elif cmd == "serverinfo":
        command = "-serverinfo"
    elif cmd == "example":
        command = "-example ranking"
    elif cmd == "info":
        command = "-info @tag_member"
    elif cmd == "kick":
        command = "-kick @tag_member\n"
        command += "I Can only kick if i have the permissions"
    elif cmd == "gif":
        command = "-gif fun\n"
        command += "I only takes the first arg of the message for the tag\n"
        command += "eg. -gif fun happy, tag: fun"
    elif cmd == "echo":
        ...
        command = "-pixiv coding is fun\n"
        command += "i will take all the args as the search key\nsearch key: 'coding is fun'"
    elif cmd == "pixivday":
        command = "-pixivday"
    elif cmd == "pixivweek":
        command = "-pixivweek"
        ...
        command = "-ranking week_original\n"
        command += "mode: [day, week, month, day_male, day_female, week_original, week_rookie, day_manga]\n"
        command += "mode r18: [day_r18, day_male_r18, day_female_r18, week_r18, week_r18g]"
    elif cmd == "pixivuser":
        command = "-pixivuser 12878330"
    elif cmd == "play":
        command = "-play asmr\n"
        command += "usage: -play <args>, args: search key or youtube url"
    elif cmd == "s":
        command = "-s asmr\n"
        command += "usage: -play <args>, args: search key\nrelated: -p, -playlist"
    elif cmd == "playlist":
        command = "-playlist\nplaylist will be updated everytime -s command has been called"
    elif cmd == "p":
        command = "-p 1\nplays song number 1 on playlist\nto check playlist, use -playlist"
    else:
        command = "Unknown command. Sorry, i don't know what " + cmd + " is"
